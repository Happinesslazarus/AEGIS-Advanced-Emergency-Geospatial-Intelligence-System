...
contributions for a single prediction, used by the chatbot when users
ask "why is my area flagged?".
"""
...
class FloodPredictor:
    ...
    When trained models are available in model_registry/, uses real ML predictions.
    Falls back to physics-based heuristics when no trained model exists.
    Run the training pipeline (POST /api/train) to generate real models from DB data.
    """
    ...
    async def _predict_with_model(
        ...
        try:
            ...
            # Clip to valid range and ensure native Python float
            ...
            if optimal_threshold > 0.0 and optimal_threshold < 1.0 and optimal_threshold != 0.5:
                if probability < optimal_threshold:
                    ...
                    probability = 0.5 + (probability - optimal_threshold) / (1.0 - optimal_threshold) * 0.5
                probability = float(np.clip(probability, 0.0, 1.0))

            # Confidence from model metrics.
            # Use roc_auc (primary training metric) with f1_macro as fallback.
            roc_auc = metadata.performance_metrics.get('roc_auc', 0.0)
            f1 = metadata.performance_metrics.get('f1_macro', 0.0)
            raw_conf = roc_auc if roc_auc > 0 else f1 if f1 > 0 else 0.65
