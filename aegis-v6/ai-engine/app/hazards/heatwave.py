"""
Heatwave risk scorer targeting heat-health emergencies.  Weighs maximum
temperature, humidity, and urban heat-island index to produce a risk
level aligned with UK Met Office amber/red heat alerts.

Unlike the other hazard models this one applies a population-vulnerability
modifier (elderly %, green-space ratio) when the demographic dataset is
available, otherwise falls back to pure meteorological scoring.
"""
...
class HeatwavePredictor:
    ...
    def __init__(self, model_registry: ModelRegistry, feature_store: FeatureStore):
        ...
        self._label_strategy = "Trained on historical heatwave event records"
    ...
    async def predict(self, request: PredictionRequest) -> PredictionResponse:
        ...
        if model_metadata:
            ...
            if model:
                ...
                self._model_type_label = "rule-based"
        else:
            probability, confidence = self._stub_prediction(features)
            self._model_type_label = "rule-based"
    ...
    async def _predict_with_model(self, model: Any, features: Dict[str, float], metadata: Any) -> Tuple[float, float]:
        ...
        try:
            ...
            _thr = float((metadata.extra_metadata or {}).get('optimal_threshold', 0.5))
            if 0.0 < _thr < 1.0 and _thr != 0.5:
                probability = (probability / _thr * 0.5) if probability < _thr else (0.5 + (probability - _thr) / (1.0 - _thr) * 0.5)
            probability = float(np.clip(probability, 0.0, 1.0))
