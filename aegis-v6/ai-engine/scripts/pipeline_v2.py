...
def moondream_safety_gate(image_path, timeout=60):
    """Binary safe/unsafe using moondream vision via Ollama."""
    with open(image_path, "rb") as f:
        img_b64 = base64.b64encode(f.read()).decode("utf-8")

    payload = {
        "model": MOONDREAM_MODEL,
        "prompt": "Is this a disaster scene? Yes or no.",
        "images": [img_b64],
        "stream": False,
    ...
    try:
        ...
        elif "yes" in raw_lower or "disaster" in raw_lower or "danger" in raw_lower:
            return "unsafe", raw, elapsed
        elif "no" in raw_lower or "safe" in raw_lower or "normal" in raw_lower:
            return "safe", raw, elapsed
        else:
            return "unsafe", raw, elapsed
...
def run_pipeline(skip_gemma=False):
    ...
    for img_data in images:
        ...
        if not img_file.exists():
            ...
            if alt.exists():
                img_file = alt
        if img_file.exists():
            img_paths[img_id] = img_file

    print(f"Found {len(img_paths)}/{total} image files\n")

    # PHASE 1: Moondream safety gate (BEFORE loading CLIP on GPU)
    print("=" * 70)
    print("  PHASE 1: Moondream Safety Gate (no CLIP on GPU yet)")
    print("=" * 70)
    ...
    for i, img_data in enumerate(images):
        ...
        if img_id not in img_paths:
            print(f"  [{i+1:2d}/{total}] {img_id}: SKIP", flush=True)
            continue
        safety, raw, ms = moondream_safety_gate(img_paths[img_id])
        moondream_cache[img_id] = (safety, raw, ms)
        expected = img_data["expected_type"]
        exp_safe = "safe" if expected == "safe" else "unsafe"
        ok = "OK" if safety == exp_safe else "XX"
        print(f"  [{i+1:2d}/{total}] {img_id}: {ok} gate={safety:6s} exp={exp_safe:6s} ({ms}ms) {raw[:60]}", flush=True)

    gate_safe = sum(1 for v in moondream_cache.values() if v[0] == "safe")
    gate_unsafe = sum(1 for v in moondream_cache.values() if v[0] == "unsafe")
    print(f"\n  Gate results: {gate_safe} safe, {gate_unsafe} unsafe\n")

    # PHASE 2: CLIP Classification (now load on GPU)
    print("=" * 70)
    ...
    for i, img_data in enumerate(images):
        ...
        if img_id not in img_paths:
            continue
        preds = clip.classify(img_paths[img_id])
        clip_cache[img_id] = preds

    print(f"  Classified {len(clip_cache)} images\n")

    # PHASE 3: Decision Logic + Gemma3 Reasoning
    layers = "L1(Moondream) + L2(CLIP)"
    if not skip_gemma:
        layers += " + L3(Gemma3)"

    print("=" * 70)
    print(f"  PHASE 3: Decision Logic ({layers})")
    print("=" * 70 + "\n")
    ...
    for i, img_data in enumerate(images):
        ...
        safety, moon_raw, moon_ms = moondream_cache[img_id]
        ...
        results.append({
            ...
            "gemma_reason": gemma_reason_text[:200] if gemma_reason_text else "",
            "gemma_ms": gemma_ms,
        })
    ...
    report_path = script_dir / "reports" / "pipeline_benchmark_3layer_v2.json"
