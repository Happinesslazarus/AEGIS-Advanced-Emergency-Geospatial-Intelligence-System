        ...
        <div className="fixed bottom-6 right-6 z-50 w-[360px] max-w-[calc(100vw-2rem)] h-[500px] max-h-[calc(100vh-6rem)] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-sm">
                {view === 'threads'
                  ...
                  : view === 'new'
                    ? 'New Conversation'
                    : (currentThread?.subject || 'Chat')}
              </span>
            </div>
            <div className="flex items-center gap-1">
              {view === 'threads' && (
                <button onClick={() => setView('new')} className="hover:bg-white/10 p-1.5 rounded-lg transition" title={'New Message'}>
                  <Plus className="w-4 h-4" />
                </button>
              )}
              <button onClick={() => setIsOpen(false)} className="hover:bg-white/10 p-1.5 rounded-lg transition" title={'Close'}>
                <X className="w-4 h-4" />
              </button>
            </div>
          </div>
            ...
            <div className="flex-1 overflow-y-auto">
              {socket.threads.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center px-6">
                  ...
                  <p className="text-sm font-medium text-gray-500 dark:text-gray-300">{'No conversations yet'}</p>
                  <p className="text-xs text-gray-400 dark:text-gray-300 mt-1">{'Start a new conversation with our team'}</p>
                  ...
                  {socket.threads.sort((a, b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime()).map(thread => (
                    <button key={thread.id} onClick={() => openThread(thread.id)}
                      ...
                      <div className="flex items-center justify-between mt-1">
                        <p className="text-[11px] text-gray-400 dark:text-gray-300 truncate max-w-[220px]">{thread.last_message || 'No messages'}</p>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        {thread.is_emergency && <span className="text-[9px] bg-red-100 text-red-700 px-1.5 py-0.5 rounded font-medium">{'Emergency'.toUpperCase()}</span>}
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-2 bg-gray-50 dark:bg-gray-950">
                {socket.messages.length === 0 && (
                  <div className="text-center text-xs text-gray-400 dark:text-gray-300 py-8">{'No messages yet'}</div>
                )}
          ...
          {view === 'new' && (
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              <div>
                <label className="block text-[10px] font-semibold text-gray-500 dark:text-gray-300 mb-1 uppercase tracking-wide">{'Subject'}</label>
              ...
              <div>
                <label className="block text-[10px] font-semibold text-gray-500 dark:text-gray-300 mb-1 uppercase tracking-wide">{'Category'}</label>
              ...
              <div>
                <label className="block text-[10px] font-semibold text-gray-500 dark:text-gray-300 mb-1 uppercase tracking-wide">{'Message'}</label>
              ...
              {newCategory === 'emergency' && (
                <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-xl p-3 flex items-start gap-2">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-red-700 dark:text-red-300">{'Emergency Thread'}</p>
                    <p className="text-[10px] text-red-600 dark:text-red-400">{'This will be flagged for immediate attention.'}</p>
                  </div>
                </div>
              )}
