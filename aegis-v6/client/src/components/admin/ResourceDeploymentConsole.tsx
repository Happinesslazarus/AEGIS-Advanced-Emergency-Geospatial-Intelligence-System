...
export default function ResourceDeploymentConsole({
  ...
  const handleDeploy = useCallback((zone: any) => {
    ...
    askConfirm(t('resource.deployResources', lang), `${t('resource.deployConfirmPrefix', lang)} ${zone.zone}? ${t('resource.deployConfirmSuffix', lang)}`, 'success', async () => {
      ...
      apiDeployResources(zone.id, user?.id, reason).then(() => { setDeployments(d => d.map(x => x.id === zone.id ? { ...x, deployed: true } : x)); pushNotification(t('resource.deploySuccess', lang), 'success') }).catch(() => pushNotification(t('resource.deployFailed', lang), 'error'))
  ...
  const handleRecall = useCallback((zone: any) => {
    ...
    askConfirm(t('resource.recallResources', lang), `${t('resource.recallConfirmPrefix', lang)} ${zone.zone}? ${t('resource.recallConfirmSuffix', lang)}`, 'warning', async () => {
      ...
      apiRecallResources(zone.id, reason, reason).then(() => { setDeployments(d => d.map(x => x.id === zone.id ? { ...x, deployed: false } : x)); pushNotification(t('resource.recallSuccess', lang), 'warning') }).catch(() => pushNotification(t('resource.recallFailed', lang), 'error'))
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="rounded-2xl overflow-hidden bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-lg">
        ...
        <div className="px-5 py-4 flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setShowShortcuts(p => !p)} className="w-8 h-8 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 flex items-center justify-center text-gray-500 dark:text-gray-300 text-xs font-mono transition-colors" title={t('resource.keyboardShortcuts', lang)}>?</button>
            ...
            {assetRows.map((a, i) => {
              ...
              return (
                <div key={i} className="rounded-xl border border-gray-100 dark:border-gray-800 p-4">
                  ...
                  <p className="text-[10px] text-gray-400 dark:text-gray-300 mt-1.5 text-right tabular-nums">{pct}% {t('resource.deployed', lang).toLowerCase()}</p>
                </div>
              )
            })}
          </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden">
        <div className="px-5 py-3 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <span className="text-[11px] text-gray-400 dark:text-gray-300 font-medium">{zones.length} {t('resource.zone', lang).toLowerCase()}{zones.length !== 1 ? 's' : ''}</span>
        ...
        {zoneView === 'table' && <div className="overflow-x-auto">
          ...
          {zones.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-gray-400 dark:text-gray-300">{t('resource.noZonesMatch', lang)}</p>
            </div>
          )}
        </div>}
