    ...
    <div className={`rounded-lg p-4 ${getSeverityBorderClass(alert.severity)} animate-fade-in`} role="alert">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-2 text-xs text-gray-500 dark:text-gray-300">
        ...
        {onDismiss && <button onClick={() => onDismiss(alert.id)} className="flex-shrink-0 text-gray-400 dark:text-gray-300 hover:text-gray-600 dark:hover:text-gray-300 dark:text-gray-300" aria-label={t('alertCard.dismiss', lang)}><X className="w-4 h-4" /></button>}
      </div>
    </div>
  )
}
