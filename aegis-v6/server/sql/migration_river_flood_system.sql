...
DO $$ BEGIN
  ALTER TABLE flood_zones ADD COLUMN IF NOT EXISTS region_id VARCHAR(50) DEFAULT 'aberdeen_scotland_uk';
  ALTER TABLE flood_zones ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT false;
  ALTER TABLE flood_zones ADD COLUMN IF NOT EXISTS activated_at TIMESTAMPTZ;
  ALTER TABLE flood_zones ADD COLUMN IF NOT EXISTS activated_by VARCHAR(100);
...
DO $$ BEGIN
  ALTER TABLE flood_predictions ADD COLUMN IF NOT EXISTS region_id VARCHAR(50) DEFAULT 'aberdeen_scotland_uk';
  ALTER TABLE flood_predictions ADD COLUMN IF NOT EXISTS valid_until TIMESTAMPTZ DEFAULT NOW() + INTERVAL '1 hour';
EXCEPTION WHEN undefined_table THEN NULL; END $$;
...
-- No explicit transaction boundary required in this migration.
