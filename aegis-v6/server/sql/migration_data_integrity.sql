...
EXCEPTION WHEN undefined_table THEN
  RAISE NOTICE 'community_chat_messages not present; skipping sender index.';
END $$;
...
BEGIN
  -- Community post author_id is NOT NULL in current schema; delete owned posts.
  DELETE FROM community_posts WHERE author_id = p_citizen_id;

  -- Anonymize community chat messages (if table exists)
  ...
  -- community_comments.author_id is NOT NULL; delete comments from deleted citizen.
  DELETE FROM community_comments WHERE author_id = p_citizen_id;
  ...
  -- community_post_reports.reporter_id is NOT NULL; delete report rows from deleted citizen.
  DELETE FROM community_post_reports WHERE reporter_id = p_citizen_id;
